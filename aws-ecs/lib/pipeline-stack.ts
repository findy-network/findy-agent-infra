  ...
  createE2ETestStep(
    ...
    return new CodeBuildStep("FindyAgencyE2ETestStep", {
      ...
      commands: [
        ...
        "aws ssm put-parameter --name \"/findy-agency-e2e/user-name\" --value $(jq '.user' ./e2e/e2e.user.json) --type String 2>&1 > /dev/null | true",
        "aws ssm put-parameter --name \"/findy-agency-e2e/org-name\" --value $(jq '.organisation' ./e2e/e2e.user.json) --type String 2>&1 > /dev/null | true",
        "aws ssm put-parameter --name \"/findy-agency-e2e/default-key\" --value $(jq '.key' ./e2e/e2e.user.json) --type String 2>&1 > /dev/null | true",
        "aws ssm put-parameter --name \"/findy-agency-e2e/cred-def-id\" --value $(jq '.credDefId' ./e2e/e2e.user.json) --type String 2>&1 > /dev/null | true",

        // existing user, new organisation
        'export E2E_USER=$(aws ssm get-parameter --name "/findy-agency-e2e/user-name" | jq .Parameter.Value)',
        'export E2E_KEY=$(aws ssm get-parameter --name "/findy-agency-e2e/default-key" | jq .Parameter.Value)',
        "npm run test:e2e",

        // existing user, existing organisation
        'export E2E_ORG=$(aws ssm get-parameter --name "/findy-agency-e2e/org-name" | jq .Parameter.Value)',
        'export E2E_CRED_DEF_ID=$(aws ssm get-parameter --name "/findy-agency-e2e/cred_def_id" | jq .Parameter.Value)',
        "npm run test:e2e",
      ],
      rolePolicyStatements: [
        new PolicyStatement({
